...
function addMessage(content, type, sources = null, isWelcome = false) {
    ...
    if (sources && sources.length > 0) {
        // Build source links as individual badge items
        const sourceItems = sources.map(source => {
            if (source.lesson_link) {
                // Create clickable link that opens in new tab
                return `<a href="${escapeHtml(source.lesson_link)}" target="_blank" rel="noopener noreferrer" class="source-badge">${escapeHtml(source.display_text)}</a>`;
            } else {
                // No link available - just display text
                return `<span class="source-badge source-badge-plain">${escapeHtml(source.display_text)}</span>`;
            }
        }).join('');
